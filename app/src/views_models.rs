...
impl ModelsView {
    ...
    fn pair_card(
        ...
        let (chip, checked) = if m.complete {
            if m.active {
        ...
        let mut card = h_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(if checked {
                        IconName::Check.into_any_element()
                    } else {
    ...
    fn tier_panel(
        ...
        let switch: gpui_kit::AnyElement = if active {
            h_flex()
                .gap_1()
                .child(div().child(format!("{label} pair active")).font_weight(FontWeight::SEMIBOLD))
        ...
        let mut panel = v_flex()
            ...
            .child(Self::pair_card(store, &pair_s, theme, hc, cx))
            .child(Self::pair_card(store, &pair_l, theme, hc, cx))
            .child(switch);
        if !vlm_id.is_empty() {
            panel = panel
                .child(
                ...
                .child(Self::pair_card(store, &vlm_id, theme, hc, cx));
    ...
    fn model_row(
        ...
        let size = format!("{:.1} GB", m.bytes as f64 / 1073741824.0);
        let (chip, checked) = if m.complete {
            if m.active { ("active", true) } else { ("ready", false) }
        } else if m.present {
            ("partial", false)
        } else {
            ("missing", false)
...
impl Render for ModelsView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let rows_stt: Vec<_> = stt_models
            .iter()
            .map(|m| self.model_row(m, &theme_id, hc, cx).into_any_element())
            .collect();
        ...
        let body = v_flex()
            ...
            .child(theme::value(active_pair, &theme_id, hc))
            .child(theme::hint("Compute: see Settings → Processing.", &theme_id, hc))
            .child(Self::tier_selector(&tier, &self.store))
            .child(Self::tier_panel(&self.store, &tier, &theme_id, hc, cx))
            .child(
                theme::hint(
                    "Linked models load straight from Ollama's store — no copies, no re-downloads.",
                    &theme_id,
                    hc,
                ),
            )
            ...
            // Side-by-side STT | LLM | Vision columns (each keeps a usable
            // minimum so narrow windows stack them one per row).
            .child(
                h_flex()
                    .gap_4()
                    .flex_wrap()
