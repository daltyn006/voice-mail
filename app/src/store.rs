...
impl FileState {
    /// Human word for the UI (never `{:?}` — enum names are debug output).
...
/// Canonicalize a persisted theme id to one of `theme::THEME_ORDER`.
/// Unknown (or legacy `high_contrast`) values fall back to dark.
pub fn normalize_theme_mode(s: &str) -> String {
    match s {
        "light" | "gruvbox_dark" | "gruvbox_light" | "coffee_dark" | "coffee_light" => {
            s.to_string()
        }
        _ => "dark".to_string(),
    }
...
/// Canonicalize the navigation chrome id ("tabs" | "sidebar").
pub fn normalize_nav_mode(s: &str) -> String {
    if s == "sidebar" {
        "sidebar".to_string()
    } else {
        "tabs".to_string()
...
pub struct Store {
    ...
    /// Theme mode: one of `crate::theme::THEME_ORDER`
    /// ("dark" | "light" | "gruvbox_dark" | "gruvbox_light" | "coffee_dark" |
    /// "coffee_light"). Persisted in ui.json.
    pub theme_mode: String,
    /// High-contrast overlay on the base theme (Settings-only switch).
    /// Persisted in ui.json; migrated from legacy `theme_mode`.
    pub high_contrast: bool,
    /// Navigation chrome: "tabs" | "sidebar" (Settings → Appearance).
    /// Persisted in ui.json; instant switch, no restart.
    pub nav_mode: String,
    /// Navigation generation counter (drives the page-swipe animation id).
    pub nav_seq: u64,
    /// Dark theme on (legacy; superseded by theme_mode). Persisted in ui.json.
    pub dark_mode: bool,
    /// Reduce/eliminate UI animations for accessibility. Persisted in ui.json.
...
impl Store {
    ...
    pub fn goto_page(&mut self, page: Page) {
        ...
        if self.page != page {
            self.page = page;
            // Generation counter for the page-swipe enter animation: each
            // navigation replays it (ids keyed by sequence always remount).
            self.nav_seq += 1;
        }
    }
    ...
    fn apply_prefs(&mut self, prefs: pv_backend::prefs::UiPrefs) {
        // Pre-overlay files stored contrast as a theme value: fold it into
        // the overlay flag so the choice survives the upgrade.
        let (theme_mode, high_contrast) = if prefs.theme_mode == "high_contrast" {
            ("dark".to_string(), true)
        } else {
            (normalize_theme_mode(&prefs.theme_mode), prefs.high_contrast)
        };
        self.theme_mode = theme_mode;
        self.nav_mode = normalize_nav_mode(&prefs.nav_mode);
        self.high_contrast = high_contrast;
        self.dark_mode = !crate::theme::ThemeModeExt::from_str(&self.theme_mode).is_light();
        self.reduce_motion = prefs.reduce_motion;
    ...
    /// Set base theme ("dark" | "light" | "gruvbox_dark" | "gruvbox_light" |
    /// "coffee_dark" | "coffee_light"; unknown → "dark"). Callers must apply
    /// via `theme::apply_theme` after (see Settings + ToggleTheme) — the
    /// store holds the value, kit globals update only through apply.
    pub fn set_theme_mode(&mut self, mode: &str) {
        let mode = normalize_theme_mode(mode);
        self.theme_mode = mode;
        self.dark_mode =
            !crate::theme::ThemeModeExt::from_str(&self.theme_mode).is_light();
        self.save_prefs();
        self.status = format!("Theme: {}.", self.theme_mode);
    }

    /// Advance to the next base theme in the Ctrl-T cycle. Returns the new
    /// (theme_mode, high_contrast) so the caller can `apply_theme` it.
    pub fn cycle_theme(&mut self) -> (String, bool) {
        let next = crate::theme::next_theme(&self.theme_mode).to_string();
        self.set_theme_mode(&next);
        (self.theme_mode.clone(), self.high_contrast)
    }

    /// Toggle the high-contrast overlay (Settings-only switch). Returns the
    /// new (theme_mode, high_contrast) for `apply_theme`.
    pub fn set_high_contrast(&mut self, on: bool) -> (String, bool) {        self.high_contrast = on;
        self.save_prefs();
        self.status = if on {
    ...
    /// Switch navigation chrome ("tabs" | "sidebar", Settings → Appearance).
    /// Persisted; takes effect immediately, no restart.
    pub fn set_nav_mode(&mut self, mode: &str) {
        self.nav_mode = normalize_nav_mode(mode);
        self.save_prefs();
        self.status = format!("Navigation: {}.", self.nav_mode);
    }

    /// Toggle reduce motion for accessibility (persisted).
    pub fn set_reduce_motion(&mut self, on: bool) {
        ...
        self.status = if on {
            ...
            "Reduced motion disabled.".to_string()
        };
    }
...
mod tests {
    ...
    fn nav_mode_normalizes_and_persists() {
        ...
        .unwrap();
        let mut s = Store::new();
        assert_eq!(s.nav_mode, "tabs");
        assert_eq!(normalize_nav_mode("sidebar"), "sidebar");
        assert_eq!(normalize_nav_mode("bogus"), "tabs");
        s.set_nav_mode("sidebar");
        assert_eq!(s.nav_mode, "sidebar");
        s.set_nav_mode("bogus");
        assert_eq!(s.nav_mode, "tabs");
    }
