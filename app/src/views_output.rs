...
use gpui_kit::{
    div, px, rgba, App, Context, Entity, FontWeight, IntoElement, Render, Subscription, Window,
};
...
impl OutputView {
    ...
    fn row(&self, o: &OutFile, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mut row = h_flex().gap_2().flex_wrap().p_1()
            ...
            .rounded(px(6.0));
        if is_open {
            row = row.bg(rgba(theme::surface(&theme_id, hc)));
        }
        row.child(
            div()
                .flex_1()
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        body = body.child(
            h_flex()
                ...
                .child(
                    theme::value(
                        format!("{count} file(s) in output (static snapshot)."),
                        &theme_id,
                        hc,
                    ),
                ),
...
impl OutputView {
    ...
    fn render_review(&mut self, mid: u64, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let (target, theme_id, hc, dirty, merged) = {
            let snap = self.store.read(cx);
        ...
        let rows: Vec<_> = naive_diff(&ltext, &rtext)
            ...
            .map(|(t, text)| {
                let color = match t {
                    '+' => rgba(theme::success(&theme_id, hc)),
                    '-' => rgba(theme::danger(&theme_id, hc)),
                    _ => rgba(theme::muted(&theme_id, hc)),
                };
                div()
                    .truncate()
