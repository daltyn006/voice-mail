...
// voice mail — native GPUI entry point.
...
fn show_crash_popup(text: &str) {
    ...
    let text = wide(text);
    let caption = wide("voice mail has stopped working");
...
// Lucide icons embedded in the binary (only the used set — the full
// Lucide collection would add ~1.5MB). Registered below via
// `with_assets`; without registration every `IconName` renders empty.
icon_assets!(VmAssets, [
    AudioLines,
    Check,
    Clapperboard,
    Copy,
    ...
    application()
        ...
        cx.spawn(async move |cx| {
            ...
            cx.open_window(opts, |window, cx| {
                ...
                let theme_mode = store_read.theme_mode.clone();
                let high_contrast = store_read.high_contrast;
                let _ = store_read;
