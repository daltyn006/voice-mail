...
fn waveform(peaks: &[f32], theme: &str, hc: bool, light_base: bool, clips: u64) -> impl IntoElement {
    ...
    let bars: Vec<_> = peaks
        ...
        .map(|p| {
            ...
            div()
                ...
                .h(px(h))
                .bg(rgba(if hot {
                    theme::danger(theme, hc) // clipping must shout on every theme
                } else if light_base {
                    theme::muted(theme, hc) // quiet gray bars on light
                } else {
                    theme::info(theme, hc) // calm steel-blue on dark
                }))
        })
        .collect();
    v_flex()
        ...
        .child(if clips > 0 {
            h_flex()
                ...
                .child(format!("{clips} clip(s) — move the mic back"))
                .into_any_element()
        } else {
            div().child("levels look good").into_any_element()
        })
}
...
impl Render for RecordView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let (theme_id, hc, light_base) = (
            snap.theme_mode.clone(),
            snap.high_contrast,
            crate::theme::ThemeModeExt::from_str(&snap.theme_mode).is_light(),
        );
        let store = self.store.clone();

        let mut body = v_flex().gap(px(theme::SPACE_LG)).p(px(theme::SPACE_XL));
        body = body
            .child(theme::title("Record — voice capture (24-bit WAV)", &theme_id, hc))
            .child(theme::value(line, &theme_id, hc))
