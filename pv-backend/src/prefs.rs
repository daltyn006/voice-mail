...
pub struct UiPrefs {
    /// Base theme: "dark" (default), "light", "gruvbox_dark",
    /// "gruvbox_light", "coffee_dark", or "coffee_light".
    #[serde(default = "default_theme_mode")]
    pub theme_mode: String,
    /// Navigation chrome: "tabs" (top underline tabs, default) or
    /// "sidebar" (left icon menu). Instant switch, no restart.
    #[serde(default = "default_nav_mode")]
    pub nav_mode: String,
    /// High-contrast overlay on top of the base theme (maxes out
    /// text/border/label contrast, keeps base background hues).
    #[serde(default = "default_false")]
    pub high_contrast: bool,
    /// Dark theme on (legacy; superseded by theme_mode).
    #[serde(default = "default_true")]
    pub dark_mode: bool,
    /// Reduce/eliminate UI animations for accessibility.
    #[serde(default = "default_false")]
...
fn load_with_migration(path: &Path) -> UiPrefs {
    ...
    let mut prefs: UiPrefs = text
        ...
        .and_then(|t| serde_json::from_str(t).ok())
        .unwrap_or_default();
    // Pre-overlay files stored contrast as a theme value: fold it into the
    // new overlay flag so the user's high-contrast choice survives.
    if prefs.theme_mode == "high_contrast" {
...
mod tests {
    ...
    #[test]
    fn legacy_high_contrast_theme_migrates_to_overlay() {
        let p = tmp("hc-migrate");
        std::fs::write(&p, r#"{"theme_mode":"high_contrast"}"#).unwrap();
        let back = load_with_migration(&p);
