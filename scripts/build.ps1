#Requires -Version 7
# One-go build for Present Voice (native GPUI app, no Tauri).
...
# ---- 3. Rust workspace (GPUI app + backend lib) ----
    ...
    if ($Dev) {
      ...
      cargo test -p present-voice --bins
      Write-Host 'Done. Binary: target/debug/present-voice.exe (dev build, do not ship)'
    } else {
      ...
      cargo test -p present-voice --bins --release
      Write-Host 'Done. Binary: target/release/present-voice.exe'
    }
