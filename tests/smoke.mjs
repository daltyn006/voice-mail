...
// Missing files record a failure (never throw — a stack trace hides the
// rest of the suite and CI can't report which contract broke).
const src = (p) => {
  try { return readFileSync(p, 'utf8'); }
  catch (e) { ok(false, `read ${p}: ${e.message}`); return ''; }
};
...
ok(src('app/src/views_output.rs').includes('odel-file-'), 'trash button deletes row + files');
...
ok(!src('app/src/views.rs').includes('🔒'), 'no nav lock — processing merged into input');
  ok(src('app/src/views.rs').includes('"Input"') && src('app/src/views.rs').includes('"Settings"'), 'nav covers Input..Settings');
...
ok(src('app/Cargo.toml').includes('assets/voice-mail.ico'), 'installer icon wired');
...
ok(existsSync('assets/voice-mail.ico'), 'source voice-mail.ico in assets/');
...
ok(existsSync('assets/app.rc') && src('assets/app.rc').includes('voice-mail.ico'), 'icon resource file points at voice-mail.ico');
...
ok(src('app/src/main.rs').includes('with_assets'), 'icon assets registered at boot');
for (const f of ['views_input.rs','views_record.rs','views_output.rs','views_models.rs','views_settings.rs','views_wizard.rs']) {
  const body = src(`app/src/${f}`);
  ok(!/[⚠⏸▶■🗑✓⏭↻☑☐❚🎵📄🎬]/.test(body), `${f}: no font-glyph icons`);
}
...
ok(!src('app/src/views_input.rs').includes('{:?}'), 'no debug-format file state in UI');
ok(src('app/src/theme.rs').includes('SPACE_XL') && src('app/src/views_settings.rs').includes('SPACE_XL'), 'spacing scale codified and used');
// Palette values are 0xRRGGBBAA: rgb() drops the first byte (blue/pink
// shift bug), so manual surfaces must go through rgba().
for (const f of ['views.rs','views_input.rs','views_models.rs','views_output.rs','views_record.rs','views_settings.rs','views_wizard.rs']) {
  const body = src(`app/src/${f}`);
  ok(!body.includes('rgb(theme::'), `${f}: no rgb() on palette values`);
  ok(!/rgb\(0x[0-9a-fA-F_]{7,}\)/.test(body), `${f}: no 8-digit rgb() literal`);
}
ok(existsSync('assets/voice-mail.png') && existsSync('assets/voice-mail.ico'), 'program icon set present (png + multi-size ico)');
ok(src('.github/workflows/release.yml').includes('pull_request'), 'CI runs on PRs');
ok(src('.github/workflows/release.yml').includes('needs: [supply-chain]'), 'release blocked on failed audit');
ok(src('.github/workflows/release.yml').includes('Mock gate'), 'tags never ship mock transcription');
ok(src('scripts/make-icon.ps1').includes('voice_mail.jpg'), 'icon build is reproducible from voice_mail.jpg');
