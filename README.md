# voice mail — batch transcribe + summarize (.exe)
...
> **AI assistance disclosure:** this project is built with AI coding
> assistants (autocomplete, code review, and agentic development) under
> human direction. All changes are reviewed, tested (`cargo test`,
> `node tests/smoke.mjs`), and committed by the maintainer.
...
- `voice-mail_*_x64-setup.exe` (NSIS, **per-machine** — needs admin) → `C:\Program Files\voice mail`, everything in that one folder (installer grants Users write access for good measure).
- `voice-mail_*_x64_en-US.msi` (WiX, **per-user**) → same folder, same contents.

Writable data (models, outputs, `app.db`) always lives under `%LOCALAPPDATA%\voice mail`, so standard users never hit "access denied" regardless of installer.
